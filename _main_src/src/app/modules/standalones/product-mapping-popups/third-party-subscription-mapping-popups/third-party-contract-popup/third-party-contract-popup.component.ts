...
@Component({
  selector: 'app-third-party-subscribtion-contract-mapping-pop-up',
  standalone: true,
  ...
  templateUrl: './third-party-contract-popup.component.html',
  styleUrl: './third-party-contract-popup.component.scss'
})
export class ThirdPartySubscribtionContractMappingPopUpComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.ExternalCustomerId = this.ExternalCustomerId
      ...
      this.contractDatatableConfig = {
        ...
        ajax: (dataTablesParameters: any, callback: any) => {
          ...
          const searchParams = {
            ...
            customerId: this.ExternalCustomerId,
            Name: this.name,
            StartInd,
            PageSize:length,
            EndInd,
          ...
          .pipe(takeUntil(this.destroy$)).subscribe(({ Data }: any) => {
              ...
              if (Data.length > 0) {
                ...
                if (this.c3Customers.AgreementId != null || this.c3Customers.AgreementId != undefined) {
                  Data = Data.map((elm: any) =>({     
                    ...elm,
                    selectedContract: elm.Id === this.c3Customers.AgreementId          
                  }));
                  this.selectedContract = _.find(this.agreements,(e: any ) => {e.Id == this.c3Customers.AgreementId}); 
                }
              }
              callback({
                data: Data,
  ...
  updateSelectedList(item: any) {
    ...
    this.c3Customers.selectedContract = item
  }

  Submit() {
    if (!this.c3Customers.selectedContract) {
      ...
      let resultData = this.c3Customers;
