...
@Component({
  selector: 'app-bundles-basic-details',
  templateUrl: './bundles-basic-details.component.html',
  styleUrl: './bundles-basic-details.component.scss'
})
export class BundlesBasicDetailsComponent extends C3BaseComponent implements OnInit {
  ...
  //$rootScope.SelectedResellerSettings.DefaultTermsAndConditionURL;
  isDataLoaded = false;
  ...
  getProductDetails(product: any) {
    ...
    this._subscription = this._manageProductService.getProductDetails(product.InternalCustomerProductId).subscribe((response: any) => {//ajmal:todo: Nexted subscription
      ...
      this.showCancelButton = ((this.isCustomerAllowedToReduceSeats === true || this.isInheritedPartnerRole != false) && this.product.IsCustomerLevel && this.product.CanCancel && this.product.Status != 'Expired' && this.product.Status != 'Suspended' && this.Permissions.HasCancelProductSubscription && this.product.Status != 'Deleted') && this.product.IsBundleContainNCEProduct;
      ...
      let obj = { EntityName: this._commonService.entityName, RecordId: this._commonService.recordId, ProviderProductId: this.product.ProviderProductId }
      ...
      this._notifierService.confirm({ title: confirmationMessage }).then((result: any) => {
        if (result.isConfirmed) {
          const subscription = this._manageProductService.setAutoRenewMode(product.InternalCustomerProductId, product.IsAutoRenew).pipe(takeUntil(this.destroy$)).subscribe({
            ...
            error: (error: any) => {
              const message = this._translateService.instant('TRANSLATE.'+error.error.ErrorMessage) ;
