...
export class NgxSummernoteDirective
  ...
  private updateEditor(content: any) {
   ...
   content = unwrapSafeValue(content as SafeValue);
  ...
  private initListeners() {
    ...
    // when code view is changed back and forth update the ngModel
    this._$element.on('summernote.codeview.toggled', (
    event,
    contents,
    $editable
    ) =>{
      // ST: issue: when in code view the latest content isnt updated to ngModel and hence when saving we get old value 
      // to make sure changes in codeview editor is immediately reflected make sure that
      // we attach a event listener to the text area of the codeview editor
      // grab the value from the event object when change event is detected on the codeview textarea
      // use functions which are already defined to update ngmodel & content
          this._$element
          .summernote(this._options)
          .data('summernote').modules.codeview.$codable.on('change',function(e){

            // get the value from event object
            var contentFromCodeViewEditor = e.target?.value || '';
            setTimeout(function () {
              self.updateModel(contentFromCodeViewEditor);
            }, 0);
