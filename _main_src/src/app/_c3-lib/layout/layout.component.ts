...
export class LayoutComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.ShowFooterAcrossAllPages = appSettingService.$rootScope.settings !== undefined && appSettingService.$rootScope.settings !== null ? appSettingService.$rootScope.settings?.ShowFooterAcrossAllPages : "false";
    config.container = 'body';
    // define layout type and load layout
      const subscr =
      merge(
        translationService.customNavigationEnd$,          // normal navigation
        this.router.events    // replay after language ready
      ...
      switchMap((event)=>{
        if (event instanceof NavigationEnd) {
          ...
          return this.layout.layoutConfigSubject.asObservable()
        }
        else{
          return of(null)
        } 
      ...
      .subscribe((config) => { 
        if(config!=null && this.loader.isLayoutLoaded){
  ...
  private initLayoutSubscriptions(): void {
  // Layout config updates
  const layoutSub = this.layout.layoutConfigSubject
    .asObservable()
    .subscribe(config => {
      // No need for setTimeout once CSS is ready
      this.updateProps(config);
      this.appReadyService.markReady();
      this.loader.isLayoutLoaded = true;
    });

  this.unsubscribe.push(layoutSub);

  // Public access / side menu visibility
  const menuSub = this.menuService.$isPublicAccess
    .subscribe(res => {
      this.showSideMenu = !res;
      this.cdref.detectChanges();
    });
  this.unsubscribe.push(menuSub);
}
