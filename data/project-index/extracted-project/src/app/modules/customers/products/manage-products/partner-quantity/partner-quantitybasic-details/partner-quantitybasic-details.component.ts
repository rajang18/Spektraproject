...
@Component({
  selector: 'app-partner-quantitybasic-details',
  standalone: false,
  templateUrl: './partner-quantitybasic-details.component.html',
  styleUrl: './partner-quantitybasic-details.component.scss'
})
export class PartnerQuantityBasicDetailsComponent extends C3BaseComponent implements OnInit {
        ...
        const subscription = this._manageProductService.suspendSubscription(product.InternalCustomerProductId, { PONumber: this.PONumber, TermsAndConditionsUrl: defaultTerms }).pipe(
          ...
          if (response.Status === 'Success') {
            if(product.IsImmediateProvisioning && (product.IsImmediateProvisioning.toLowerCase() === 'no' || product.IsImmediateProvisioning.toLowerCase() === 'false')){
              this.readyToComplete = true;
              this._toastService.success(this._translateService.instant('TRANSLATE.CUSTOMER_PRODUCT_MANAGE_PAGE_NOTIFICATION_SUSPEND_REQUEST_SUBMITTED_SUCCESSFULLY', { productName: product.ProductSubscriptionName }));
          ...
          this._subscriptionArray.push(subscription);
        // this._manageProductService.suspendSubscription(product.InternalCustomerProductId, { PONumber: this.PONumber, TermsAndConditionsUrl: nceTerms })
      }
    })
  }
  ...
  cancelSubscription(product: any) {
    ...
    if (HasCancelationWindowClosed) {
      ...
      this._notifierService.confirm({ title: confirmationMessage }).then((result) => {
        if (result.isConfirmed) {
          this.readyToComplete = false;
          this._subscription = this._manageProductService.cancelSubscription(product.InternalCustomerProductId, { PONumber: this.PONumber, TermsAndConditionsUrl: defaultTerms }).pipe(
            catchError((err) => {
              let errmsg: string = this._translateService.instant('TRANSLATE.' + err.error.ErrorMessage);
              this._toastService.error(errmsg, {
                timeOut: 5000
