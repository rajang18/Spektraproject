...
@Component({
  selector: 'app-purchase-product-mapping-contract-popup',
  standalone: true,
  ...
  templateUrl: './purchase-product-mapping-contract-popup.component.html',
  styleUrl: './purchase-product-mapping-contract-popup.component.scss'
})
export class PurchaseProductMappingContractPopupComponent implements OnInit, OnDestroy {
  ...
  Submit() {
    if (this.selectedContract == 0) {
      let text = this.activeServiceDetail == 'Connectwise' ? this._translateService.instant('TRANSLATE.MAPPING_WARNING_TEXT_NO_AGREEMENT_IS_SELECTED') : this._translateService.instant('TRANSLATE.MAPPING_WARNING_TEXT_NO_CONTRACT_IS_SELECTED')
      this._notifierService.confirm({ title: text })
    } else {
