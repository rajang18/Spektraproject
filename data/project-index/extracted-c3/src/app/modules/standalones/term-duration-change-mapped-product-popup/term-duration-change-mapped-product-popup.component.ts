...
@Component({
  selector: 'app-term-duration-change-mapped-product-popup',
  standalone: true,
  imports: [CommonModule, NgSelectModule, FormsModule, ReactiveFormsModule, TranslationModule, NgbModule, Select2Module],
  templateUrl: './term-duration-change-mapped-product-popup.component.html',
  styleUrl: './term-duration-change-mapped-product-popup.component.scss'
})
export class TermDurationChangeMappedProductPopupComponent extends C3BaseComponent implements OnInit, OnDestroy {
    ...
    this.subscriptionsList.forEach((value: any) => {
      ...
      value.MappingC3PlanProducts.forEach((planProduct: any) => {
        this.mappingC3PlanProductsDataset.push({
          ...
          data: { PlanOfferQualifiedOfferName: planProduct.PlanOfferQualifiedOfferName, BillingTypeName: planProduct.BillingTypeName, PlanName: planProduct.PlanName, CurrencySymbol: planProduct.CurrencySymbol, FinalSalePrice: planProduct.FinalSalePrice, CompositeProductId: planProduct.CompositeProductId }
        });
      })
  ...
  selectPlanProductId(row: any, planProduct: any) {
    ...
    this.selectedOffer = planProductSelectedValue;
