...
@Component({
  selector: 'app-integration-details-view-log',
  templateUrl: './integration-details-view-log.component.html',
  styleUrl: './integration-details-view-log.component.scss'
})
export class IntegrationDetailsViewLogComponent implements OnInit, OnChanges {
        ...
        columns: [
          ...
          },
          {
            ...
            render: (data: string, type: any, row: any, meta: any) => {
              ...
              const formattedLogMessage = data
                ? data.replace('ERROR_DESC_FAILED_DUE_TO_BAD_REQUEST', translatedError)
                : '';

              const formattedErrorDetails = row?.ErrorDetails
                ? row.ErrorDetails.replace('ERROR_DESC_FAILED_DUE_TO_BAD_REQUEST', translatedError)
                : '';

              if ((row && row.LogType == 'Error') && (row.Status == 'Failed')) {
                return `<span class="text-danger">${formattedLogMessage}</span>&nbsp;<span>${formattedErrorDetails}</span>`;
              }
              else if ((row && row.LogType == 'Error') && (row.Status != 'Failed')) {
                return `<span class="text-danger">${formattedLogMessage}</span>`;
              }
              else {
                return `<span>${formattedLogMessage}</span>`;
              }
      }
    }
        ],
