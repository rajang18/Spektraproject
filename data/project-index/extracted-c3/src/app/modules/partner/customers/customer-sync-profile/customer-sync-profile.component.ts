...
@Component({
  selector: 'app-customer-sync-profile',
  templateUrl: './customer-sync-profile.component.html',
  styleUrl: './customer-sync-profile.component.scss'
})
export class CustomerSyncProfileComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this._pageInfo.updateTitle(this._translateService.instant("TRANSLATE.CUSTOMER_BUSY_MESSAGE_RELOAD_PROVIDER_CUSTOMER_PROFILES"),true);
    this._pageInfo.updateBreadcrumbs(['SIDEBAR_TITLE_MENUS_SELL_DIRECT','PARTNER_DASHBOARD_CUSTOMER_TILE_INTRO_TITLE','CUSTOMER_BUSY_MESSAGE_RELOAD_PROVIDER_CUSTOMER_PROFILES']);
  }
  ...
  loadData(CustomerIndex: number) {
    ...
    const subscription = this._customersListingService.loadData(customerC3Id).pipe(takeUntil(this.destroy$)).subscribe((response: any) => {
