import { ChangeDetectorRef, Component, ComponentFactoryResolver, ElementRef, EventEmitter, Input, OnChanges, OnInit, Renderer2, SimpleChanges, TemplateRef, ViewChild, ViewContainerRef } from '@angular/core';
...
import { ReportPopupConfig, MODAL_DIALOG_CLASS } from 'src/app/shared/models/report-popup.model';
...
@Component({
    selector: 'app-business-nested-list',
    templateUrl: './business-nested-list.component.html',
    styleUrl: './business-nested-list.component.scss'
})
export class BusinessNestedListComponent extends C3BaseComponent implements OnInit {
    ...
    @Input() selectedBillingPeriodId: any[] = [];
    ...
    ngOnInit(): void {
        this.aLLPeriods = this.translateService.instant("TRANSLATE.SELECT_DATE_VALUE");
        ...
        let selectedBillingPeriodIds = localStorage.getItem("SelectedBillingPeriodForRevenueVsCostSummary");
        if (selectedBillingPeriodIds != null && selectedBillingPeriodIds != undefined && selectedBillingPeriodIds != '') {
            this.selectedBillingPeriodId = JSON.parse(selectedBillingPeriodIds);
    ...
    toggleAllRows() {
        ...
        table.rows().every(function (rowIdx, tableLoop, rowLoop) {
            ...
            if (row?.data()?.isShowInvoice) {
                // self.fetchChildlineItemsForSummaryView(row, row.data());
            } else {
                self.calculateNestedData(row, row.data());
            }
        });
    }
    ...
    HasPermission() {
        ...
        this.Permissions.HasCustomerAndResellerDetails = this.permissionService.hasPermission(CloudHubConstants.GETCUSTOMERANDRESELLERDETAILSFORBULKINVOICEDOWNLOAD);
    ...
    showSelectedFirstPeriod() {
        ...
        if (index != -1) {
            return (
                ...
                '(' +
                this.billingPeriodsData[index]?.BillingId +
                ')'
            );
        } else {
    ...
    calculateNestedData(row: any, selectedObj: any) {
        ...
        this.groupByResult = this._buisnessService?.groupByResult || this.groupByResult
    ...
    //         billingPeriods?.Data?.map((item: any) => {
    //           item['BillingStartDate'] = moment(item['BillingStartDate']).format(this.globalDateFormat);
    //           item['BillingEndDate'] = moment(item['BillingEndDate']).format(this.globalDateFormat);
    //           item['BillGenerationDate'] = moment(item['BillGenerationDate']).format(this.globalDateFormat);
    //           return item;
    //         }).reverse() || [];
    //       this.billingPeriodId =
    ...
    // }
    handleTableConfig() {
        this.applyEscapeHTML(this.data);
        let data = this.data;
        let isShowInvoice: boolean = false;
        data.forEach((item: any) => {
            if (item.isShowInvoice) {
                isShowInvoice = true;
            }
        })
        let self = this;
        ...
        if (isShowInvoice && this.isAllselected) {
            columnArr.shift();
        }
        setTimeout(() => {
        ...
        modalRef.result.then((reason) => {
            if (reason) {
                ...
                this.customerInvoiceReloadingList.forEach((row: any) => {
                    const subscription = this._buisnessService.getInvoices(row.Entity, row.C3Id, this.billingPeriodId, this.invoiceGenerateReason).pipe(takeUntil(this.destroy$)).subscribe((response: any) => {
    ...
    exportProfitability() {
        ...
        for (var i = 0; i < this.selectedAggType.length; i++) {
            aggregateColumns = aggregateColumns + this.selectedAggType[i].Value;
            if (i !== this.selectedAggType.length - 1) {
        ...
        let req = {
            AggregateColumns: aggregateColumns,
            BillingPeriodIds: this.selectedBillingPeriodId[0],
    ...
    getMonthlyTurnoverReport() {
        this.fileService.getFile(`reports/${this.commonService.entityName}/${this.commonService.recordId}/BillingPeriod/${this.billingPeriodId}/TurnoverReport/Download`)
    }

    getBillingForecastReport() {
        this.fileService.getFile(`reports/BillingPeriod/${this.billingPeriodId}/BillingForecastReportForQuantity`)
        this.fileService.getFile(`reports/BillingPeriod/${this.billingPeriodId}/BillingForecastReportForUsage`)
    }

    getInvoiceLineItemsWithTaxesReport = function () {
        this.fileService.getFile(`reports/${this.commonService.entityName}/${this.commonService.recordId}/BillingPeriod/${this.billingPeriodId}/InvoiceLineItemsWithTaxesReport`)
    }

    getInvoiceLineItemsWithCommissionsReport() {
        this.fileService.getFile(`reports/${this.commonService.entityName}/${this.commonService.recordId}/BillingPeriod/${this.billingPeriodId}/InvoiceDetailsWithCommissions/Download`)
    }
    ...
    reloadData() {
        ...
        if (this.selectedBillingPeriodId.length == 0) {
            this.toastService.error(
                this.translateService.instant('TRANSLATE.BILLING_PERIOD_SELECT_VALIDATION_ERROR'));
            return;
        }
