        ...
        <div class="tabs">
          <a
            ...
            *ngFor="let tab of data.tabs"
            ...
            routerLink="/test-case-generator"
        ...
        <div class="panel-body" [ngSwitch]="view()">
          <section *ngSwitchCase="'tests'">
            ...
            <table class="data-table">
              <thead>
                <tr><th>Test Case ID</th><th>Test Case Description</th><th>Expected Result</th></tr>
              </thead>
              <tbody>
          ...
          <section *ngSwitchCase="'overview'">
            ...
            <div class="artifact-grid">
              <a class="artifact-card" *ngFor="let artifact of artifacts" routerLink="/test-case-generator" [queryParams]="{ view: artifact.view }">
          ...
          <section *ngSwitchCase="'jira'">
            ...
            <table class="data-table">
              <thead><tr><th>Task ID</th><th>Task Title</th><th>Priority</th><th>Status</th></tr></thead>
              <tbody>
...
export class TestCaseGeneratorPageComponent {
  private readonly route = inject(ActivatedRoute);
  private readonly query = toSignal(this.route.queryParamMap);

  readonly data = generatedModuleMock;
  readonly fields = this.data.fields.map((field) => ({ ...field }));
  readonly view = computed<ArtifactView>(() => (this.query()?.get('view') as ArtifactView | null) ?? 'tests');
  ...
  readonly artifacts = [
    { view: 'form', icon: 'badge', title: 'Angular Form Preview', summary: `${this.data.fields.length} fields configured` },
    { view: 'api', icon: 'api', title: 'API Contract Preview', summary: `${this.data.endpoints.length} endpoints` },
    { view: 'jira', icon: 'hub', title: 'Jira Task Generator', summary: `${this.data.jiraTasks.length} tasks` },
    { view: 'tests', icon: 'fact_check', title: 'Test Case Generator', summary: `${this.data.testCases.length} cases` },
    { view: 'code', icon: 'code', title: 'Code Preview', summary: `${this.data.codeFiles.length} files` }
